...
def main():
    ...
    parser.add_argument('--csv-file', type=int, default=0)
    ...
    for beta1 in beta1s:
        for e in epochs:
            for g_l in g_loss_lambdas:
                ...
                dt_train = readData.ReadPhysionetData(args.data_path, os.path.join(args.data_path, "train", "list.txt"),
                                                      isNormal=args.isNormal, isSlicing=args.isSlicing,
                                                      readCsv=args.csv_file)
                dt_test = readTestData.ReadPhysionetData(args.data_path,
                                                         os.path.join(args.data_path, "test", "list.txt"),
                                                         dt_train.maxLength, isNormal=args.isNormal,
                                                         isSlicing=args.isSlicing, readCsv=args.csv_file)
                tf.reset_default_graph()
