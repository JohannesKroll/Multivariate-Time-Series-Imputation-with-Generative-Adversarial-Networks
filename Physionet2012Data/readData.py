...
class ReadPhysionetData():
    ...
    # produce next batch
    def __init__(self, dataPath, labelPath, isNormal, isSlicing, sliceGap=60, readCsv=False):
        print("data path: " + dataPath)
        if readCsv:

            start_date = datetime.strptime('2006-12-01', "%Y-%m-%d")
            self.dataPath = dataPath + '_train.csv'
            self.dic = {'time': -1}
            self.fileNames = []
            self.x = []
            self.times = []
            ...
            with open(self.dataPath) as csvfile:
                ...
                for h in headers:
                    if h != "company_id" and h != "date":
                        self.dic[h] = count
                        count += 1

                self.mean = [0.0] * (len(self.dic) - 1)
                self.meancount = [0] * (len(self.dic) - 1)
                current_company = ''
                company_data = []
                company_times = []
                ...
                for row in reader:
                    ...
                    for col in range(2, len(row)):
                        data[col-2] = float(row[col])
                        self.mean[col-2] += float(row[col])
                        self.meancount[col-2] += 1
            ...
            self.meancount = [0] * (len(dic) - 1)
            x = []  # List of data lists
            times = []  # years submitted
            non_in_dic_count = 0
            ...
            for fileName in fileNames:
                    ...
                    if count > 1:
                        ...
                        if timestamp == "00:00":
                            if feature == 'Age':
                                ...
                                # calcuate mean
                                if age != "0":
                                    self.mean[self.dic[feature]] += float(age)
                                    self.meancount[self.dic[feature]] += 1
                            if feature == 'Gender':
                                if value == "-1":
                                    gender = "0"
                                if value == "0":
                                ...
                                # calcuate mean
                                if gender != "0":
                                    self.mean[self.dic[feature]] += float(gender)
                                    self.meancount[self.dic[feature]] += 1
                            if feature == 'Height':
                                height = "0" if value == "-1" else value
                                # calcuate mean
                                if height != "0":
                                    self.mean[self.dic[feature]] += float(height)
                                    self.meancount[self.dic[feature]] += 1
                            if feature == 'ICUType':
                                icutype = "0" if value == "-1" else value
                                # calcuate mean
                                if icutype != "0":
                                    self.mean[self.dic[feature]] += float(icutype)
                                    self.meancount[self.dic[feature]] += 1
                            if feature == 'Weight':
                                weight = "0" if value == "-1" else value
                                # calcuate mean
                                if weight != "0":
                                    self.mean[self.dic[feature]] += float(weight)
                                    self.meancount[self.dic[feature]] += 1
                        else:
                            if timestamp != lastTime:
                                ...
                                self.mean[self.dic[feature]] += float(value)
                                self.meancount[self.dic[feature]] += 1
                ...
                x.append(totalData)  # all data points for one company being appended
                times.append(t_times)  # years of company
                f.close()
        ...
        for i in range(len(self.mean)):
            if self.meancount[i] != 0:
                self.mean[i] = self.mean[i] / self.meancount[i]

        # normalization
        m = []  # mask 0/1
        # first calculate std
        self.std = [0.0] * (len(self.dic) - 1)
        ...
        for j in range(len(self.std)):
            self.std[j] = math.sqrt(1.0 / (self.meancount[j] - 1) * self.std[j])
        ...
        # print(len(self.m))
        # print("self.deltaPre")
        # print(len(self.deltaPre))
        # print("self.lastvalues")
        # print(len(self.lastvalues))
        # print("self.deltaSub")
        # print(len(self.deltaSub))
        # print("self.subvalues")
        # print(len(self.subvalues))
        # print("self.x_lengths")
        # print(self.x_lengths)
        ...
        for i in range(len(self.mean)):
            resultFile.writelines(str(self.mean[i]) + "," + str(self.std[i]) + "," + str(self.meancount[i]) + "\r\n")
        resultFile.close()
    ...
    def nextBatch(self):
        ...
        while i * self.batchSize <= len(self.x):
                ...
                for h in range(0, self.x_lengths[j]):
                    if h == 0:
                        ...
                        try:
                            ...
                            pass
                            # TODO: problem lies here. not all company times are equal
            ...
            if self.isNormal:
                yield x, y, [0.0] * (
                            len(self.dic) - 1), m, deltaPre, x_lengths, lastvalues, files, imputed_deltapre, imputed_m, deltaSub, subvalues, imputed_deltasub
            else:
...
if __name__ == '__main__':
    ...
    dt = ReadPhysionetData("/home/yonghong/ImputationAndPredictionUsingGAN/set-a/train",
                           "/home/yonghong/ImputationAndPredictionUsingGAN/set-a/train/list.txt", isNormal=True,
                           isSlicing=True)
