...
class ReadPhysionetData():
    ...
    # produce next batch
    def __init__(self, dataPath, labelPath, maxLength, isNormal, isSlicing, sliceGap=60, readCsv=False):
        print("data path: " + labelPath)
        if readCsv:

            start_date = datetime.strptime('2006-12-01', "%Y-%m-%d")
            self.dataPath = dataPath + '_test.csv'
            self.dic = {'time': -1}
            self.fileNames = []
            self.x = []
            self.times = []
            ...
            with open(self.dataPath) as csvfile:
                ...
                mean = [0.0] * (len(self.dic) - 1)
                meancount = [0] * (len(self.dic) - 1)
                current_company = ''
                company_data = []
                company_times = []
